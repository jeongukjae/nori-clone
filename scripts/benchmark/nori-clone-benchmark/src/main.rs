...
fn main() {
    ...
    let system_dictionary =
        SystemDictionary::load_from_input_directory(args.dict.as_str()).unwrap();
    let user_dictionary = UserDictionary::load_from_input_directory(
        args.dict.as_str(),
        &system_dictionary.connection_cost.additional,
    )
    .unwrap();
